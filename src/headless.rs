...
impl HeadlessSession {
    pub fn new(seed: u64) -> Self {
        ...
        // The single seeded RNG for the whole session. Inserted before
        // `generate_farms` runs so the initial farm layout draws from `--seed`,
        // and shared with `advance_month_system` so every monthly roll continues
        ...
        // These only need `Commands` (and, for `generate_farms`, the seeded
        // `GameRng`), so just run the real startup systems directly rather than
        // duplicating their logic in a pure variant.
        app.world_mut()
